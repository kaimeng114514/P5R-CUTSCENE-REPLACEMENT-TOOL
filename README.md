...
![版本](https://img.shields.io/badge/version-1.3.0-red)
...
- **P5 风格 UI** —— 红黑配色、不规则矩形按钮、自绘滚动条、自定义字体
...
4. 如需取消，选中后点击 **"撤销替换"**
5. 点击 **"一键生成 Mod"**，输入 Mod 名称和简介，选择输出目录
6. 点击 **"启动 Reloaded-II"**，启用你的 Mod，启动游戏即可
...
**Q：预览没声音 / 声音不对？**
- v1.3.0 已修复 HCA 加密音频识别问题，全部 103 个过场均可正常播放声音。
- 双语过场（如 MOV001）已默认播放日文配音。
- 如果你是从旧版本升级，且之前生成过预览缓存，请点击 **"清除所有记录并重置"** 后重新导入 CPK，让缓存以新逻辑重新生成。
...
| 视频编码 | VP9（IVF，stream 0）；后段过场（MOV090~097）为 MPEG-2 |
| 音频编码 | HCA（加密存储，需解密后解码）；后段过场为 ADX |
| 双音轨 | 日文配音 = 最后一条 HCA，替换时需显式指定 `channel_no` |
...
- **HCA 加密音频**：P5R USM 中 HCA 音频以加密形式存储，`stream_bytes()` 返回的原始字节前 4 字节不是明文 `HCA\x00`。必须通过 `hca.decode()` 内部解密后才能正确识别和解码。直接用魔数匹配会导致 93 个普通过场音频被跳过。
- **双音轨 `channel_no`**：双音轨过场（如 MOV001）替换时，必须使用 `UsmMuxConfig + UsmMuxAudioTrack` 显式指定 `channel_no=0,1`，否则 CRI 解码器会提前判定流结束，导致视频播放约 14.5 秒后被强制截断。
